...
Sprite   GP_ONE_0::spriteMemory[MAX_SPRITE_COUNT];
uint16_t GP_ONE_0::frameBuffer[FRAMEBUFFER_BUF_SIZE];

void GP_ONE_0::loadSprites(const Sprite *sprites, uint16_t spriteCount) {
...
void GP_ONE_0::clearFrameBuffer(BackGroundColor bkgColor) {
...
void GP_ONE_0::drawSpriteInstances(uint16_t command_size,const SpriteInstance *instances, uint16_t instanceCount) {
...
void GP_ONE_0::saveFrameBuffer(FrameBuffer &outFrameBuffer) {
    for (uint16_t i = 0; i < FRAMEBUFFER_BUF_SIZE; i++)
